...
void DronePwmReceiver::setThrottleYawPitchRoll(Drone *drone)
{
    ...
    {
        float desired_yaw_angle = map(receiver.getChannelValue(yaw_receiver_channel_number), 1060, 1900, -60, 60);
        drone->setDesiredYawAngle(desired_yaw_angle);

        float desired_pitch_angle = map(receiver.getChannelValue(pitch_receiver_channel_number), 1060, 1900, -60, 60) * -1;
        drone->setDesiredPitchAngle(desired_pitch_angle);

        float desired_roll_angle = map(receiver.getChannelValue(roll_receiver_channel_number), 1060, 1900, -60, 60);
        drone->setDesiredRollAngle(desired_roll_angle);
